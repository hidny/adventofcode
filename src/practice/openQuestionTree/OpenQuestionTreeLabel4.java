...
public class OpenQuestionTreeLabel4 {

	//TODO: don't use longs...
	public static void main(String[] args) {
		// TODO Auto-generated method stub

		String perm[] = new String[21];
		...
		for(long p=0; p<numIterations; p++) {
			...
			for(int j=0; j<curPerm.length; j++) {
				...
				if(j > 0) {
					...
					int curDiff = Math.abs(pint(curPerm[j]) - pint(curPerm[(j+3)/4 - 1]));
					...
					/*if(j % 4 != 1 && j > 0 && pint(curPerm[j-1]) > pint(curPerm[j])) {
