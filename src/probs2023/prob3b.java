...
public class prob3b {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int gearsFound[][] = new int[lines.size()][lines.get(0).length()];
			long ratioThing[][] = new long[lines.size()][lines.get(0).length()];
			for(int i=0; i<ratioThing.length; i++) {
				for(int j=0; j<ratioThing[0].length; j++) {
					ratioThing[i][j] = 1;
				}
			}
