...
public class prob4B {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int numCopiesPrev[] = new int[lines.size()];
			...
			for(int i=0; i<lines.size(); i++) {
				...
				int curNumCopies = numCopiesPrev[i];
				line = lines.get(i);
				...
				for(int j=i+1; j<=i+numMatches && j<lines.size(); j++) {
					numCopiesPrev[j] = numCopiesPrev[j] + numCopiesPrev[i];
				}
