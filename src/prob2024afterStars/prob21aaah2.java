	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int i=0; i<lines.size(); i++) {
				...
				String shortestOld = getShortestSequence(new prob21state2(1), line);
				...
				String actualbefore = getShortestSequence(new prob21state2(3), line);
				String actual = getShortestSequence(new prob21state2(4), line);
				
				actualbefore = rearrange(actualbefore);
				String baseOnMatrix = getLongerSolutionBasedOnMatrix(actualbefore);
				...
				sopl("baseOnMatrix:    " + baseOnMatrix);
				sopl("baseOnMatrix:    " + baseOnMatrix.length());
				...
				prob21state2 curState = new prob21state2(4);

				String curOutput = "";
				for(int i2=0; i2<actual.length(); i2++) {
					curState = curState.makeMove(getMoveIndex(actual.charAt(i2)));
					...
					if(curState.curOutput.length() > curOutput.length()) {
						sopl(actual.substring(0, i2 + 1));
						sopl(curState.curOutput);
						sopl(actual.substring(0, i2 + 1).length());
						curOutput = curState.curOutput;
					}
					
				}
				sopl("output " + ": (" + curState.curOutput + ")");
				

				curState = new prob21state2(4);
				
				curOutput = "";
				for(int i2=0; i2<baseOnMatrix.length(); i2++) {
					curState = curState.makeMove(getMoveIndex(baseOnMatrix.charAt(i2)));
					
					if(curState.badState) {
						sopl("reached bad state at i2 = " + i2);
					}
				...
				sopl("output2 matrix based " + ": (" + curState.curOutput + ")");
