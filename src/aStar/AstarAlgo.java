...
public class AstarAlgo {
	...
	public static ArrayList<AstarNode> astar(AstarNode start, AstarNode goal) {
		...
		fScoreQuickMinFinder.insert(new HeapTreeObject(start.getAdmissibleHeuristic(), start));
		...
		while(openSet.isEmpty() == false) {
			...
			for(int i=0; i<neighbours.size(); i++) {
				...
				fScoreQuickMinFinder.insert(new HeapTreeObject(tentative_gScore + currentNeighbour.getAdmissibleHeuristic(), currentNeighbour));
			}
		}
	...
	public static AstarNode getLowestFFunctionNode(HashMap<AstarNode, Long> fScoreOpenNodes, HeapTreeMT fScoreQuickMinFinder) {
		HeapTreeObject temp;
		do {
			temp = (HeapTreeObject)fScoreQuickMinFinder.extractMin();
