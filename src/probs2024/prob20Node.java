...
public class prob20Node implements AstarNode {
	...
	public static void init(boolean mapChar[][]) {
		...
		mapUsed = new prob20Node[map.length][map[0].length];
		for(int i=0; i<map.length; i++) {
			for(int j=0; j<map[0].length; j++) {
				mapUsed[i][j] = new prob20Node(i, j);
			}
		}
	}
	
	public prob20Node(int i, int j) {
		this.i = i;
		this.j = j;
	}
	//TODO: init
	public static boolean map[][];
	public static prob20Node mapUsed[][];
	public int i;
	public int j;
	
	public static int excepti = -1;
	public static int exceptj = -1;
	
	public static int goali = -1;
	public static int goalj = -1;
			...
			for(int j2=j-1; j2<=j+1; j2++) {
				if((i2 !=i && j2 == j) || (i2 ==i && j2 != j)) {
					...
					if(i2 >= 0 && j2 >= 0 && i2 <map.length && j2 <map[0].length) {
						...
						if(map[i2][j2] == false || (excepti == i2 && exceptj == j2)) {
