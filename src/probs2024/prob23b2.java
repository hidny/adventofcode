...
public class prob23b2 {
	...
	public static int[] getSingleOffIndex(String list1, String list2) {
		int ret[] = new int[2];
		
		ret[0] = -1;
		ret[1] = -1;
		
		int list1A[] = parseList(list1);
		int list2A[] = parseList(list2);
		
		int numMatches = 0;
		for(int i=0; i<list1A.length; i++) {
			for(int j=0; j<list2A.length; j++) {
				
				if(list1A[i] == list2A[j]) {
					numMatches++;
					break;
				}
			}
		...
		if(numMatches == list1A.length - 1) {
			...
			int num2 = -1;
			for(int i=0; i<list2A.length; i++) {
				boolean matches = false;
				for(int j=0; j<list1A.length; j++) {
					
					if(list2A[i] == list1A[j]) {
						matches = true;
						break;
					}
				}
