		...
		try {
			...
			while(prob13Guy.guys.size() >1) {
				...
				for(int gi=0; gi<prob13Guy.guys.size(); gi++) {
					...
					if(prob13Guy.hasCollision()) {
						int coord[] =prob13Guy.getCollision();
						sop("Collision: (x,y) = " +coord[1]+ ","+ coord[0]);
						gi = prob13Guy.removeAndGetNextIndex(coord[0], coord[1], gi);
					}
			...
			System.out.println(prob13Guy.guys.get(0).i);
			int answer2[] = {prob13Guy.guys.get(0).i, prob13Guy.guys.get(0).j};
			int y = answer2[0];
			int x = answer2[1];
			if(prob13Guy.guys.size() != 1) {
				sop("WHAT!");
			}
			//63,26
			sopl("Answer: " + x + "," + y);
