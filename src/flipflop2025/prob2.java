...
public class prob2 {
	...
	public static void main(String[] args) {
		Scanner in;
		try {
			in = new Scanner(new File("inflipflop2025/prob2025in2.txt"));
			...
			for(int i=0; i<lines.size(); i++) {
				...
				for(int j=0; j<line.length(); j++) {
					if(line.charAt(j) == '^') {
						...
						if(j == line.length() - 1 || line.charAt(j + 1) != 'v') {
							height -= fib[curInRowDown];
						}
					}
