...
public class prob19part2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			FOUND_DIST_RANGE:
			for(long i = 0; i <= LIMIT; i += (SQUARE_LENGTH - 1)) {
				for(long j=0; j<=i; j+= (SQUARE_LENGTH - 1)) {
					long x = j;
					long y = i-j;
					...
					if(ret == 1 && x >=SQUARE_LENGTH && y >= SQUARE_LENGTH) {
						sopl("here");
						if( runProg(x - (SQUARE_LENGTH - 1), y, line) == 1L && runProg(x, y - (SQUARE_LENGTH - 1), line) == 1L) {
							distRangeMax = i;
							break FOUND_DIST_RANGE;
						}
					}
				}
			...
			long distRangeMin = distRangeMax - 12 * SQUARE_LENGTH;
			
			//long distRangeMin = 0;
			//long distRangeMax = 1000;
			sopl("Distance range: " + distRangeMin + " to " + distRangeMax);
			
			FOUNDANS:
			for(dist = distRangeMin; dist<=distRangeMax; dist++) {
