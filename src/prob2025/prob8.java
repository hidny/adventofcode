...
public class prob8 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			HashSet <String> pairUsed = new HashSet<String>();
			for(int numPairs=0; numPairs< 1000; numPairs++) {
				sopl(numPairs);
				...
				for(int i=0; i<lines.size(); i++) {
					...
					for(int j=i+1; j<lines.size(); j++) {
						...
						if(! pairUsed.contains(i +"," +j) && curDist < LowestDist) {
							bestI = i;
							bestJ = j;
							LowestDist = curDist;
						}
				...
				pairUsed.add(bestI + "," + bestJ);
				pairUsed.add(bestJ + "," + bestI);
			}
			...
			for(int i=0; i<used.length; i++) {
				...
				boolean tmp[] = getSize(i, pairUsed, lines.size());
				sopl("get size");
				...
				} else if(num >secondLongest) {
					thirdLongest = secondLongest;
					secondLongest = num;
					
				} else if(num > thirdLongest) {
					thirdLongest = num;
				}
			}
