...
public class prob2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			boolean invalidIdsPart1[] = new boolean[100000];
			...
			for(int i=0; i<lines.size(); i++) {
				...
				for(int j=0; j<tokens.length; j++) {
					...
					int minNumDigits = tokens2[0].length() / 2;
					int maxNumDigits = tokens2[1].length() / 2;
					
					for(int k=(int)Math.pow(10, minNumDigits-1); k<Math.pow(10, maxNumDigits); k++) {
						
						long potNum = plong((k+"") + (k+""));
						
						if(potNum >= lhs && potNum <= rhs) {
							invalidIdsPart1[k] = true;
							sopl(potNum);
						}
					}
			...
			for(int i=0; i<invalidIdsPart1.length; i++) {
				if(invalidIdsPart1[i]) {
					sopl("???");
					cur += plong((i + "") + (i + ""));
				}
			}
