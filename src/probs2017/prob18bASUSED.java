...
public class prob18bASUSED implements Runnable{
	...
	public static void main(String[] args) {
		...
		try {
			...
			Thread thread = new Thread(new prob18bASUSED());
			thread.start();
		...
		try {
			...
			while(pc < lines.size()) {
				...
				if(token[0].equals("snd")) {
					...
					System.out.println(programNum + "Sending");
					...
					System.out.println(programNum + " RCV ");
					while(true) {
						if(programNum == 1) {
							if(sendP0.isEmpty()) {
								Thread.sleep(10);
