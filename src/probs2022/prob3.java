	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int i=0; i<lines.size(); i+=3) {
				...
				String rug1 = lines.get(i);
				String rug2 = lines.get(i + 1);
				String rug3 = lines.get(i + 2);
				
			
				// 2* (int)(c - 'A' + 1);
				boolean taken1[] = new boolean[26 * 2];
				boolean taken2[] = new boolean[26 * 2];
				boolean taken3[] = new boolean[26 * 2];
				...
				for(int j=0; j<rug3.length(); j++) {
					char c = rug3.charAt(j);
					
					if( c >= 'A' && c <= 'Z') {
						taken3[26 + (int)(c - 'A')] = true;
				...
				for(int j=0; j<26; j++) {
					if(taken1[26 + j] && taken2[26 + j] && taken3[26 + j]) {
						count +=27 + j;
						sopl(27 + j);
					}
				}
