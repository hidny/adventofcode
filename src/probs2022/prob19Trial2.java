...
public class prob19Trial2 {
					...
					for(int i=0; i<buildOrdersPrev.size(); i++) {
						...
						for(int j=0; j<robotTypes.length; j++) {
							...
							if(couldAddRobotToBuildOrder(buildOrdersPrev.get(i).buildOrder, blue.get(blueIndex), NUM_MINUTES)) {
								...
								if(numGeode == -1) {
									sopl("Invalid build order! What?");
									exit(1);
								}
								
								if(numGeode > 15) {
									sopl("Debug");
									int numGeode2 = getGeodeInValidBuildOrder(buildOrdersPrev.get(i).buildOrder, blue.get(blueIndex), NUM_MINUTES);
								...
								onlyAddIfNoDominatedAndRemoveDominated(newBuild, buildOrdersAtDepth);
	...
	public static void onlyAddIfNoDominatedAndRemoveDominated(prob19BuildOrder newBuild, ArrayList<prob19BuildOrder> buildOrdersAtDepth) {
		
		boolean removedOne = false;
		
		NEXT_COMPARE:
		for(int i=0; i<buildOrdersAtDepth.size(); i++) {
			
			prob19BuildOrder buildInList = buildOrdersAtDepth.get(i);
